...
if [ -d "src" ] || [ -d "lib" ] || [ -d "pages" ]; then
    if find . \( -name "*.js" -o -name "*.ts" -o -name "*.jsx" -o -name "*.tsx" \) -print0 | xargs -0 grep -l "validation\|validate\|sanitize\|joi\|yup\|zod" > /dev/null 2>&1; then
...
if find . -type f \( -name "*.js" -o -name "*.ts" -o -name "*.json" -o -name "*.md" \) -print0 | xargs -0 grep -l "api_key\|apikey\|password.*=\|secret.*=" 2>/dev/null | grep -v ".env.example" | grep -v "node_modules" > /dev/null; then
